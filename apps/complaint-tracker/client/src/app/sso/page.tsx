...
/**
 * Portal → Complaint Tracker SSO landing page.
...
function getApiBase(): string {
  if (typeof window === 'undefined') return process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4010/api';
  ...
  if (isLan) return `${window.location.protocol}//${host}:4010/api`;
...
function SsoHandler() {
  ...
  const runExchange = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem('complaint_auth', JSON.stringify(session));
      window.dispatchEvent(new CustomEvent('complaint:auth-refreshed', { detail: session }));
      setStage('success');
      router.replace(next);
    } catch (err) {
